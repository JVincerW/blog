<template>
    <div>
        <div class="title">
            <div>标题</div>
            <div>图片</div>
        </div>
        <div class="box">
            <div v-for="(item,index) in list" :key="index" :class="item.cls" class="b" @click="navigateToDe(item.id)">
                <div class="img">
                    <img :src=item.imageUrl style="max-width: 100%; max-height: 100%;object-fit: cover;  width: 100%;
        height: 100%;"/>
                </div>
                <div>
                    <p style="font-size: 12px; color: rgb(144, 143, 143); padding: 10px;">
                        <svg data-v-20cddaa8="" height="14" style="vertical-align: -2px;" viewBox="0 0 1024 1024" width="14">
                            <path d="M512 512m-512 0a512 512 0 1 0 1024 0 512 512 0 1 0-1024 0Z"
                                  data-v-20cddaa8="" fill="#409EFF"></path>
                            <path d="M654.222222 256c-17.066667 0-28.444444 11.377778-28.444444 28.444444v56.888889c0 17.066667 11.377778 28.444444 28.444444 28.444445s28.444444-11.377778 28.444445-28.444445v-56.888889c0-17.066667-11.377778-28.444444-28.444445-28.444444zM369.777778
                    256c-17.066667 0-28.444444 11.377778-28.444445 28.444444v56.888889c0 17.066667 11.377778 28.444444 28.444445 28.444445s28.444444-11.377778 28.444444-28.444445v-56.888889c0-17.066667-11.377778-28.444444-28.444444-28.444444z"
                                  data-v-20cddaa8=""
                                  fill="#FFFFFF"></path>
                            <path d="M725.333333 312.888889H711.111111v28.444444c0 31.288889-25.6 56.888889-56.888889 56.888889s-56.888889-25.6-56.888889-56.888889v-28.444444h-170.666666v28.444444c0 31.288889-25.6 56.888889-56.888889
                    56.888889s-56.888889-25.6-56.888889-56.888889v-28.444444h-14.222222c-22.755556 0-42.666667 19.911111-42.666667 42.666667v341.333333c0 22.755556 19.911111 42.666667 42.666667 42.666667h426.666666c22.755556 0 42.666667-19.911111 42.666667-42.666667v-341.333333c0-22.755556-19.911111-42.666667-42.666667-42.666667zM426.666667 654.222222h-56.888889c-17.066667 0-28.444444-11.377778-28.444445-28.444444s11.377778-28.444444 28.444445-28.444445h56.888889c17.066667 0 28.444
                    444 11.377778 28.444444 28.444445s-11.377778 28.444444-28.444444 28.444444z m227.555555 0h-56.888889c-17.066667 0-28.444444-11.377778-28.444444-28.444444s11.377778-28.444444 28.444444-28.444445h56.888889c17.066667 0 28.444444 11.377778 28.444445 28.444445s-11.377778 28.444444-28.444445 28.444444z m0-113.777778h-56.888889c-17.066667 0-28.444444-11.377778-28.444444-28.444444s11.377778-28.444444 28.44
                    4444-28.444444h56.888889c17.066667 0 28.444444 11.377778 28.444445 28.444444s-11.377778 28.444444-28.444445 28.444444z" data-v-20cddaa8="" fill="#FFFFFF"></path>
                        </svg>
                        <span> 发布于{{ item.createAt }}</span>
                    </p>
                    <h3 style="    white-space: nowrap;
        margin-bottom: 10px;
        text-overflow: ellipsis;
        overflow: hidden;
        margin-left: 10px;">{{ item.title }}</h3>
                    <p>
                    <span style="font-size: 12px; color: rgb(168, 168, 168); margin-left: 10px">
                        {{ item.views }}
                        <svg data-v-20cddaa8="" height="14" style="vertical-align: -2px;" viewBox="0 0 1024 1024" width="14"><path d="M14.656 512a497.344 497.344 0 1 0 994.688 0 497.344 497.344 0 1 0-994.688 0z"
                                                                                                                                   data-v-20cddaa8=""
                                                                                                                                   fill="#FF0000"></path><path d="M374.976 872.64c-48.299-100.032-22.592-157.44 14.421-211.37 40.448-58.966 51.115-117.611 51.115-117.611s31.659 41.386 19.115 106.005c56.149-62.72 66.816-162.133 58.3
                        25-200.405 127.317 88.746 181.59 281.002 108.181 423.381C1016 652.501 723.093 323.2 672.277 285.867c16.939 37.333 20.054 100.032-14.101 130.474-58.027-219.84-201.664-265.002-201.664-265.002 16.96 113.536-61.781 237.397-137.344 330.24-2.816-45.163-5.632-76.544-29.483-119.808-5.333 82.176-68.373 149.269-85.29 231.445-22.912 111.637 17.237 193.173 170.581 279.424z" data-v-20cddaa8=""
                                                                                                                                                               fill="#FFFFFF"></path></svg>
                    </span>
                        <span style="font-size: 12px; color: rgb(168, 168, 168); margin-left: 10px">
                        1500评论
                        <svg data-v-20cddaa8="" height="14" style="vertical-align: -2px;" viewBox="0 0 1024 1024" width="14"><path d="M113.834667 291.84v449.194667a29.013333 29.013333 0 0 0 28.842666 29.013333h252.928v90.453333l160.597334-90.453333h252.928a29.013333 29.013333 0 0 0 29.013333-29.013333V291.84a29.013333 29.013333 0 0 0-29.013333-29.013333h-665.6a29.013333 29.013333 0 0 0-29.696 29.013333z"
                                                                                                                                   data-v-20cddaa8=""
                                                                                                                                   fill="#FFDEAD"></path><path d="M809.130667 262.826667h-665.6a29.013333 29.013333 0 0 0-28.842667 29.013333v40.106667a29.013333 29.013333 0 0 1 28.842667-29.013334h665.6a29.013333 29.013333 0 0 1 29.013333 29.013334V291.84a29.013333 29.013333 0 0 0-29.013333-29.013333z"
                                                                                                                                                               data-v-20cddaa8=""
                                                                                                                                                               fill="#FFF3DB"></path><path
                            d="M556.202667 770.048h252.928a29.013333 29.013333 0 0 0 29.013333-29.013333V362.837333s-59.733333 392.533333-724.309333 314.709334v63.488a29.013333 29.013333 0 0 0 28.842666 29.013333h253.098667v90.453333z"
                            data-v-20cddaa8=""
                            fill="#F2C182"></path><path d="M619.008 632.32l101.888-35.157333-131.754667-76.117334 29.866667 111.274667zM891.904 148.992a61.44 61.44 0 0 0-84.138667 22.528l-19.968 34.133333 106.666667 61.610667 19.968-34.133333a61.781333 61.781333 0 0 0-22.528-84.138667z"
                                                        data-v-20cddaa8=""
                                                        fill="#69BAF9"></path><path d="M775.338667 198.775467l131.669333 76.032-186.026667 322.218666-131.6864-76.032z" data-v-20cddaa8=""
                                                                                    fill="#F7FBFF"></path><path d="M775.168 198.826667l-5.290667 9.216 59.221334 34.133333a34.133333 34.133333 0 0 1 12.458666 46.592l-139.946666 242.346667a34.133333 34.133333 0 0 1-46.762667 12.629333l-59.050667-34.133333-6.656 11.434666 88.746667 51.2L720.896 597.333333l186.026667-322.56z"
                                                                                                                data-v-20cddaa8=""
                                                                                                                fill="#D8E3F0"></path><path d="M616.448 622.592l2.56 9.728 101.888-35.157333-44.885333-25.941334-59.562667 51.370667zM891.904 148.992c-1.024 0-2.218667-0.853333-3.242667-1.536A61.610667 61.610667 0 0 1 887.466667 204.8l-19.968 34.133333-73.728-42.496-5.12 8.704 106.666666 61.610667 19.968-34.133333a61.781333 61.781333 0 0 0-23.381333-83.626667z"
                                                                                                                                            data-v-20cddaa8=""
                                                                                                                                            fill="#599ED4"></path><path d="M265.898667 417.621333H494.933333a17.066667 17.066667 0 1 0 0-34.133333H265.898667a17.066667 17.066667 0 1 0 0 34.133333zM265.898667 533.504H494.933333a17.066667 17.066667 0 0 0 0-34.133333H265.898667a17.066667 17.066667 0 0 0 0 34.133333z"
                                                                                                                                                                        data-v-20cddaa8=""
                                                                                                                                                                        fill="#3D3D63"></path><path
                            d="M959.488 354.645333a99.84 99.84 0 0 0-23.722667-127.488 78.677333 78.677333 0 0 0-142.848-64.170666l-11.605333 20.138666a17.066667 17.066667 0 0 0-20.821333 7.168l-32.085334 55.466667H142.677333a46.250667 46.250667 0 0 0-45.909333 46.08v449.194667a46.08 46.08 0 0 0 45.909333 46.08h236.032v73.386666a17.066667 17.066667 0 0 0 8.362667 14.848 17.066667 17.066667 0 0 0 8.704 2.218667 17.066667 17.066667 0 0 0 8.362667-2.218667l156.672-88.234666h248.32a46.08 46.08 0 0 0 46.08-46.08V398.677333L921.6 283.306667a17.066667 17.066667 0 0 0-4.266667-21.504l1.877334-3.413334a65.365333 65.365333 0 0 1 10.410666 79.189334l-53.077333 91.989333a56.832 56.832 0 0 0 20.821333 77.653333 17.066667 17.066667 0 0 0 24.234667-6.314666 17.066667 17.066667 0 0 0-6.997333-23.04 23.04 23.04 0 0 1-8.362667-31.061334z m-138.410667 386.389334a11.946667 11.946667 0 0 1-11.946666 11.946666H556.202667a17.066667 17.066667 0 0 0-8.362667 2.218667l-134.997333 76.117333v-61.269333a17.066667 17.066667 0 0 0-17.066667-17.066667H142.677333a11.946667 11.946667 0 0 1-11.776-11.946666V291.84a11.946667 11.946667 0 0 1 11.776-11.946667h565.930667L574.464 512a17.066667 17.066667 0 0 0-1.706667 12.970667L597.333333 615.253333H265.898667a17.066667 17.066667 0 1 0 0 34.133334h352.938666a17.066667 17.066667 0 0 0 5.802667 0l102.4-35.328a17.066667 17.066667 0 0 0 9.216-7.509334l85.333333-147.968z m-204.8-184.661334l63.829334 36.864-49.322667 17.066667z m206.848-170.666666v1.365333l-108.373333 186.709333-102.4-59.050666L781.482667 221.866667l102.4 59.050666z m76.458667-161.28L887.466667 244.224l-76.970667-44.373333 11.264-19.797334a44.544 44.544 0 1 1 77.141333 44.544z"
                            data-v-20cddaa8=""
                            fill="#3D3D63"></path></svg>
                    </span>
                        <span style="font-size: 12px; color: rgb(168, 168, 168); margin-left: 10px;">
                        {{ item.likes }}
                        <svg data-v-20cddaa8="" height="14" style="vertical-align: -2px;" viewBox="0 0 1024 1024" width="14"><path d="M510.671749 348.792894S340.102978 48.827055 134.243447 254.685563C-97.636714 486.565724 510.671749 913.435858 510.671749 913.435858s616.107079-419.070494 376.428301-658.749272c-194.095603-194.096626-376.428302 94.106308-376.428301 94.106308z"
                                                                                                                                   data-v-20cddaa8=""
                                                                                                                                   fill="#FF713C"></path><path d="M510.666632 929.674705c-3.267417 0-6.534833-0.983397-9.326413-2.950192-16.924461-11.872399-414.71121-293.557896-435.220312-529.448394-5.170766-59.482743 13.879102-111.319341 56.643068-154.075121 51.043536-51.043536 104.911398-76.930113 160.095231-76.930114 112.524796 0 196.878996 106.48115 228.475622 153.195078 33.611515-45.214784 122.406864-148.20646 234.04343-148.20646 53.930283 0 105.46603 24.205285 153.210428 71.941496 45.063335 45.063335 64.954361 99.200326 59.133795 160.920016C935.306982 641.685641 536.758893 915.327952 519.80271 926.859589a16.205077 16.205077 0 0 1-9.136078 2.815116zM282.857183 198.75574c-46.25344 0-92.396363 22.682605-137.127124 67.413365-36.149315 36.157501-51.614541 78.120218-47.25321 128.291898 17.575284 202.089671 352.199481 455.119525 412.332023 499.049037 60.434417-42.86732 395.406538-289.147446 414.567947-492.458945 4.933359-52.344159-11.341303-96.465029-49.759288-134.88199-41.431621-41.423435-85.24243-62.424748-130.242319-62.424748-122.041544 0-220.005716 152.203494-220.989114 153.742547-3.045359 4.806469-8.53335 7.883551-14.101159 7.534603a16.257266 16.257266 0 0 1-13.736863-8.184403c-0.902556-1.587148-91.569532-158.081365-213.690893-158.081364z"
                                                                                                                                                               data-v-20cddaa8=""
                                                                                                                                                               fill="#885F44"></path></svg>
                    </span>
                    </p>
                    <div style="    font-size: 13px;
        line-height: 1.4;
        overflow: hidden;
        padding: 10px;
        text-overflow: ellipsis;
        display: -webkit-box;
        -webkit-line-clamp: 2;
        -webkit-box-orient: vertical;"></div>
                    <div style="font-size: 14px; color: rgb(123, 123, 123); margin-top: 15px;">
                        <span style="margin-left: 10px;  background: rgb(220, 220, 220); padding: 5px ;border-radius:10px;">
                            <svg data-v-20cddaa8="" height="15" style="vertical-align: -3px;" viewBox="0 0 1024 1024" width="15"><path d="M179.2 153.6m89.6 0l588.8 0q89.6 0 89.6 89.6l0 486.4q0 89.6-89.6 89.6l-588.8 0q-89.6 0-89.6-89.6l0-486.4q0-89.6 89.6-89.6Z"
                                                                                                                                       data-v-20cddaa8=""
                                                                                                                                       fill="#FF9C55"></path><path d="M25.6 193.4208A91.0208 91.0208 0 0 1 116.6208 102.4H382.592a91.0208 91.0208 0 0 1 87.5008 65.9712l24.5504 85.7216h412.7488A91.0208 91.0208 0 0 1 998.4 345.1392v485.4528A91.0208 91.0208 0 0 1 907.3792 921.6H116.608A91.0208 91.0208 0 0 1 25.6 830.5792V193.408z"
                                                                                                                                                                   data-v-20cddaa8=""
                                                                                                                                                                   fill="#FFD977"></path><path
                                d="M128 665.6m25.6 0l76.8 0q25.6 0 25.6 25.6l0 0q0 25.6-25.6 25.6l-76.8 0q-25.6 0-25.6-25.6l0 0q0-25.6 25.6-25.6Z" data-v-20cddaa8="" fill="#FFF1C9"></path><path
                                d="M128 768m25.6 0l179.2 0q25.6 0 25.6 25.6l0 0q0 25.6-25.6 25.6l-179.2 0q-25.6 0-25.6-25.6l0 0q0-25.6 25.6-25.6Z" data-v-20cddaa8="" fill="#FFF1C9"></path><path
                                d="M128 486.4m51.2 0l0 0q51.2 0 51.2 51.2l0 0q0 51.2-51.2 51.2l0 0q-51.2 0-51.2-51.2l0 0q0-51.2 51.2-51.2Z" data-v-20cddaa8="" fill="#FFA86A"></path></svg>
                            {{ item.excerpt }}
                        </span>
                        <span style="margin-left: 10px;  background: rgb(220, 220, 220); padding: 5px; border-radius:10px;">
                            <svg data-v-20cddaa8="" height="15" style="vertical-align: -3px;" viewBox="0 0 1024 1024" width="15"><path d="M905.0112 560.4352l-342.784 342.784c-56.7808 56.7808-148.7872 56.7808-205.568 0l-231.5776-231.5776c-56.7808-56.7808-56.7808-148.7872 0-205.568l342.9376-342.9376a114.8928 114.8928 0 0 1 84.224-33.5872l266.3936 7.2192c60.7744 1.6384 109.7216 50.3808 111.5648 111.1552l8.2944 267.8272c1.024 31.6928-11.1104 62.3104-33.4848 84.6848z"
                                                                                                                                       data-v-20cddaa8=""
                                                                                                                                       fill="#8C7BFD"></path><path d="M675.2256 491.4688c-82.176 0-149.0432-66.8672-149.0432-149.0432s66.8672-149.0432 149.0432-149.0432 149.0432 66.8672 149.0432 149.0432-66.8672 149.0432-149.0432 149.0432z m0-192.2048c-23.808 0-43.2128 19.3536-43.2128 43.2128 0 23.808 19.3536 43.2128 43.2128 43.2128 23.808 0 43.2128-19.3536 43.2128-43.2128s-19.4048-43.2128-43.2128-43.2128z"
                                                                                                                                                                   data-v-20cddaa8=""
                                                                                                                                                                   fill="#FFE37B"></path></svg>
                            被偷走的那几年
                        </span></div>
                </div>
            </div>
        </div>

    </div>
</template>
<script>

import {v4 as uuidv4} from 'uuid';
// import Bottom from '@/components/Bottom.vue'
import {getbloglis} from '../http/home/homeApi';
// import { getclass } from '../http/home/homeApi'
//博客列表
export default {
    data() {
        return {
            list: []
        }
    },
    // component:{
    //     Bottom
    // },
    mounted() {
        this.getList();
    },
    methods: {
        async getList() {
            const res = await getbloglis();


            this.list = res.data.rows;


            this.list.forEach(async (item) => {
                item.cls = uuidv4();
            })
            console.log(this.list);
            let height = window.innerHeight;

            window.addEventListener('scroll', () => {
                this.list.forEach((item) => {
                    let dom = document.getElementsByClassName(`${item.cls}`)[0];
                    if (dom) {
                        let h = dom.getBoundingClientRect().top;

                        if (h <= height) {
                            item.cls = "l"
                        }
                    }

                })
            })
        },
        navigateToDe(id) {
            this.$router.push({path: '/De', query: {id: id}});
        }
    }
}
// onBeforeUnmount(() => {
//    window.removeEventListener('scroll', handleScroll);
// })
</script>
<style scoped>
@font-face {
    font-family: 'fontregular';
    src: url('../assets/font/XianYanBaoXiaoGou-2.ttf');
}

.img {
    height: 50%;
    width: 100%;


}

.title {
    width: 100%;
    margin: 20px;
    display: flex;
    justify-content: space-between;
    border-bottom: 2px dashed rgb(128, 128, 128);
    font-size: 20px;
}

.box {
    display: flex;
    min-width: 1000px;
    flex-wrap: wrap;
    font-family: fontregular;
}

.b {
    margin-bottom: 10px;
    width: 31%;
    float: left;
    background: rgb(255, 255, 255);
    position: relative;
    border-radius: 10px;
    overflow: hidden;
    margin-left: 20px;
    margin-top: 10px;
    min-width: 200px;
    height: 350px;
    box-shadow: 2px 2px 4px 4px rgba(0, 0, 0, 0.1);
    cursor: pointer;
}

.l {
    animation: zoomIn .8s ease-in-out;
    width: 31%;
    float: left;
    background: rgb(255, 255, 255);
    position: relative;
    border-radius: 10px;
    overflow: hidden;
    margin-left: 20px;
    margin-top: 10px;
    min-width: 200px;
    height: 350px;
    box-shadow: 2px 2px 4px 4px rgba(0, 0, 0, 0.1);
    cursor: pointer;
}

@keyframes zoomIn {
    0% {
        transform: scale(0);
    }
    100% {
        transform: scale(1);
    }
}
</style>
